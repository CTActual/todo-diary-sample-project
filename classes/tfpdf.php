...
class tFPDF
{
	...
	protected $CurOrientation;     // current orientation
		  // standard page sizes in pts
	protected $StdPageSizes = array('a3'=>array(841.89, 1190.55), 'a4'=>array(595.28, 841.89), 'a5'=>array(420.94, 595.28), 'letter'=>array(612, 792), 'legal'=>array(612, 1008) );
...
public function __construct($orientation='P', $unit='in', $size='letter')
{
	// Some checks
	$this->_dochecks();
	...
	// Page sizes
	$pg_dimensions = $this->_getpagesize($size);
	$this->DefPageSize = $pg_dimensions;
	$this->CurPageSize = $pg_dimensions;
	...
	if ($orientation=='p' || $orientation=='portrait')
	{
		...
		$this->w = $pg_dimensions[0];
		$this->h = $pg_dimensions[1];
	}
	elseif ($orientation=='l' || $orientation=='landscape')
	{
		$this->DefOrientation = 'L';
		$this->w = $pg_dimensions[1];
		$this->h = $pg_dimensions[0];
	}
	else
	...
	// Interior cell left margin in user units -- will go past right cell boundary (0)
	$this->cMargin = 0;
...
public function AddPage($orientation=null, $size=null, $rotation=0)
{
	// Start a new page
...
public function AddFont($family, $style=null, $file=null, $uni=false, $dir=null)
{
	...
	if (empty($file) ) 
	{
		if ($uni) {$file = str_replace(' ','',$family) . strtolower($style) . '.ttf';}
		else {$file = str_replace(' ','',$family) . strtolower($style) . '.php';}
		}
	...
	else 
	{
		...
		if(empty($dir) ) {$dir = $this->fontpath;}
...
public function SetFontPath($path=null)
{
...
public function Cell($cellwidth=0, $cellheight=0, $txt=null, $border=0, $ln=0, $align=null, $fill=false, $link=null)
{
	/*
	 * 	$this->x, $this->y (current x,y coordinates in user units)
	 *  $this->k (scale factor -- number of points used in user unit scale)
	 *  $this->ws (word spacing)
	 *  $this->w, $this->h (page width and height in user units)
	 * 
	 */
	 
	// Output a cell
	$txt = (!empty($txt) && !is_array($txt) ) ? $txt : null;
	$k = $this->k;
	
	if ( ($this->y + $cellheight) > $this->PageBreakTrigger && !$this->InHeader && !$this->InFooter && $this->AcceptPageBreak() )
	{
		// Automatic page break
		$x = $this->x;
		$ws = $this->ws;
	...
	if ($fill || $border == 1)
	{
		...
		$s = sprintf('%.2F %.2F %.2F %.2F re %s ', ($this->x)*$k, ( ($this->h) - ($this->y) )*$k, $cellwidth*$k, -$cellheight*$k, $op);
		}
	...
	if (is_string($border) )
	{
		...
		$hyk = ($this->h - $y)*$k;
		$xwk = ($x+$cellwidth)*$k;
		$hyhk = ($this->h - ($y+$cellheight) )*$k;
		
		if (strpos($border,'L') !== false) {$s .= sprintf($f, $xk, $hyk, $x*$k, $hyhk);}
		if (strpos($border,'T') !== false) {$s .= sprintf($f, $xk, $hyk, $xwk, $hyk);}
		if (strpos($border,'R') !== false) {$s .= sprintf($f, $xwk, $hyk, $xwk, $hyhk);}
		if (strpos($border,'B') !== false) {$s .= sprintf($f, $xk, $hyhk, $xwk, $hyhk);}
		}
	...
	if (!empty($txt) || $txt == 0)
	{
		...
		if ($align=='R') {$dx = $cellwidth - ($this->cMargin) - ($this->GetStringWidth($txt) );}
		elseif ($align=='C') {$dx = $cellwidth - ($this->GetStringWidth($txt) )/2;}
		...
		if ($this->ws && $this->unifontSubset) 
		{
			...
			$s .= sprintf('BT 0 Tw %.2F %.2F Td [', ( ($this->x) + $dx)*$k, ($this->h - ( ($this->y) + .5*$cellheight + .3*($this->FontSize) ) )*$k);
			$t = explode(' ',$txt);
			$numt = count($t);
		...
		else 
		{
			...
			else {$txt2='(' . $this->_escape($txt) . ')';}
			
			$s .= sprintf('BT %.2F %.2F Td %s Tj ET', ( ($this->x) + $dx)*$k, ( ($this->h) - ( ($this->y) + .5*$cellheight + .3*($this->FontSize) ) )*$k, $txt2);
			}
			
		if ($this->underline) {$s .= ' ' . $this->_dounderline( ($this->x) + $dx, ($this->y) + .5*$cellheight + .3*($this->FontSize), $txt);}
		if ($this->ColorFlag) {$s .= ' Q';}
		if ($link) {$this->Link( ($this->x) + $dx, ($this->y) + .5*$cellheight - .5*($this->FontSize), $this->GetStringWidth($txt), $this->FontSize, $link);}
		}
	
	if ($s) {$this->_out($s);}
		
	$this->lasth = $cellheight;
...
public function Write($lineheight=null, $txt=null, $link=null)
{
	/*
	 * 	$this->x, $this->y (current x,y coordinates in user units)
	 *  $this->k (scale factor -- number of points used in user unit scale)
	 *  $this->ws (word spacing)
	 *  $this->w, $this->h (page width and height in user units)
	 * 	$this->lMargin;            // left margin
		$this->tMargin;            // top margin
		$this->rMargin;            // right margin
		$this->bMargin;            // page break margin
		$this->cMargin;            // cell margin
	 */
	...
	{
		// Get next character
		if ($this->unifontSubset) {$c = mb_substr($s, $i, 1, 'UTF-8');}
		else {$c = $s[$i];}
		
		if ($c == "\n")
		{
			// Explicit line break
			if ($this->unifontSubset) {$this->Cell($w, $lineheight, mb_substr($s, $j, $i-$j, 'UTF-8'), 0, 2, '', false, $link);}
			else {$this->Cell($w, $lineheight, substr($s, $j, $i-$j), 0, 2, '', false, $link);}
		...
		if ($l > $wmax)
		{
			...
			{
				if ($this->unifontSubset) {$this->Cell($w, $lineheight, mb_substr($s, $j, $sep-$j, 'UTF-8'), 0, 2, '', false, $link);}
				else {$this->Cell($w, $lineheight, substr($s, $j, $sep-$j), 0, 2, '', false, $link);}
	...
	if ($i != $j) 
	{
		if ($this->unifontSubset) {$this->Cell($l, $lineheight, mb_substr($s, $j, $i-$j, 'UTF-8'), 0, 0, '', false, $link);}
		else {$this->Cell($l, $lineheight, substr($s, $j), 0, 0, '', false, $link);}
		}
...
public function Output($dest=null, $name=null, $isUTF8=false)
{
...
protected function _dochecks()
{
	...
	if (!function_exists('mb_strlen') ) {$this->Error('mbstring extension is not available');}
	}
...
protected function _getpagesize($size)
{
	...
	if (in_array($size, array('a3', 'a4', 'a5', 'letter', 'legal') ) )
	{
...
protected function _beginpage($orientation=null, $size=null, $rotation=0)
{
	$this->page++;
	...
	if (empty($orientation) ) {$orientation = $this->DefOrientation;}
	else {$orientation = strtoupper($orientation[0]);}
		
	if (empty($size) ) {$size = $this->DefPageSize;}
...
protected function _putTTfontwidths($font=array(), $maxUni) 
{
	if (isset($font['unifilename']) ) {$fn = $font['unifilename'] . '.cw127.php';} else {$fn = '';}
	...
	for ($cid=$startcid; $cid<$cwlen; $cid++) 
	{
		...
		if (!isset($font['dw']) || (isset($font['dw']) && $width != $font['dw']) ) 
		{
			if ($cid == ($prevcid + 1) ) 
			{
				if ($width == $prevwidth) 
				{
					if ($width == $range[$rangeid][0]) {$range[$rangeid][] = $width;}
					else 
					{
